...
**SQL**
列に別名をつけるメリット
- 長い列名を短くわかりやすくしたり、状況に応じて任意で内容を表示できる

アスタリスクによる前列検索は便利だが、データベースの設計変更で列が増減すると検索結果も変化してしまう、予期せぬバグの原因になることがある。開発現場では極力使用を避け、使う場合は十分な検討が必要。

**UPDATE文の基本構文**
```sql
UPDATE テーブル名
    SET 列名1 = 値1, 列名2 = 値2⋯ --SET句
    （WHERE修飾） --WHERE句がないと、どの行を書き換えるかの指定がなく、全部の行を書き換えてしまう
```

```sql
DELETE
    FROM テーブル名
    （WHERE修飾） --WHERE句がないと、どの行を削除するかの指定がなく、全部の行を削除してしまう
```

```sql
INSERT INTO テーブル名
        （列名1, 列名2, 列名3⋯）
    VALUES (値1, 値2, 値3⋯) --VALUES句
--INSERT文は、テーブルに新しいデータを１行だけ追加する命令。どの列にどのような値を追加するか、具体的に指定する構造のため、WHERE句は書けない（不要）。

INSERT INTO 家計簿
        （費目, 日付, 出金額） --ここの順序は自由だが、VALUES句と対応した順番で並べる必要がある
    VALUES ('通信費', '2024-02-20', 6200)

--2行目の列指定を丸ごと省略した場合、3行目に記述する値は、テーブルにおける列の順序と同じでなければならない。
